function [PredRotations, votedAngle]= ComputeAnglesFromProjs_SE_wvote_nonlinear(clstack, corrstack, p, method, sigma,  a)
%
...
K=size(clstack,2);
[maxh, ang] = computeMaxh_nonlinear(clstack, corrstack, K, p, method, sigma,  a);
% 新增这两行
%[maxh_wvse, ~] = computeMaxh_wvote(clstack, corrstack, K, 15);
%maxh = maxh_wvse;
...
%p_emb = 15;
%thresh = prctile(maxh(:), p_emb);
%a_soft = 20;  % 软门控陡度参数，可调
%soft_gate = 1 ./ (1 + exp(-a_soft * (maxh - thresh)));
%Pfilter = maxh .* soft_gate;
